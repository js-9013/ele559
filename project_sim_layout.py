...
#layout_path(TOP, sourceLayer, [-chipWidth/2*ex+50*ex - upperWGBondPadHeight/2*ey, -chipWidth/2*ex+50*ex +upperWGBondPadHeight/2*ey], 0)
#layout_path(TOP, port1Layer, [-chipWidth/2*ex+250*ex-lowerWGBondPadHeight/2*ey, -chipWidth/2*ex+250*ex+lowerWGBondPadHeight/2*ey], 0)
#layout_path(TOP, port2Layer, [-chipWidth/2*ex+11000*ex - 250*ex - lowerWGBondPadHeight/2*ey, -chipWidth/2*ex+11000*ex-250*ex+lowerWGBondPadHeight/2*ey], 0)
...
testRegion = pya.Region()
layout_path(TOP, port1Layer, [-chipWidth/2*ex+cavityFeedlineEnd - 15*ex - 1.5*ey, -chipWidth/2*ex+cavityFeedlineEnd - 15*ex + 1.5*ey], 0)
...
layout_path(TOP, sourceLayer, [-chipWidth/2*ex+cavityStraightEnd + 10*ex - 3 * ey, -chipWidth/2*ex+cavityStraightEnd +10*ex + 3 * ey], 0)
layout_path(TOP, port2Layer, [-chipWidth/2*ex+20*ex+cavityStraightEnd - 1.5*ey, -chipWidth/2*ex+20*ex+cavityStraightEnd + 1.5*ey], 0)
...
centerTranslation = pya.DCplxTrans(1, 0, False, -chipWidth/2*1000, 0)
feedlineRegion = feedlineRegion.transformed(centerTranslation)
#TOP.shapes(outputLayer).insert(feedlineRegion)
tFeed = pya.DCplxTrans(1, 180, False, 11000, 0)
translatedFeed = feedlineRegion.transformed(tFeed)
#TOP.shapes(outputLayer).insert(translatedFeed)

lowZBraggRegion = lowZBraggRegion.transformed(centerTranslation)
...
highZBraggRegion = highZBraggRegion.transformed(centerTranslation)
...
cavityFeedlineRegion = cavityFeedlineRegion.transformed(centerTranslation)
#TOP.shapes(outputLayer).insert(cavityFeedlineRegion)
tCavity = pya.DCplxTrans(1, 180, False, 11000-21000, 0)
translatedCavity = cavityFeedlineRegion.transformed(tCavity)
#TOP.shapes(outputLayer).insert(translatedCavity)

cavityRegion = cavityRegion.transformed(centerTranslation)
