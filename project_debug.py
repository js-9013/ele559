...
outputDebugLayer = pya.LayerInfo(1,1)
...
debugRegionUpper = pya.Region()
debugRegionLower = pya.Region()

cavityStraightBegin = cavityStraightEnd - braggBendRadius*ex - braggBendRadius*ey
bendPoints = [cavityStraightEnd, cavityStraightEnd - braggBendRadius*ex, cavityStraightBegin]
upper_shape = layout_waveguide_from_points(TOP, workingLayer, bendPoints, upperPinW, braggBendRadius)
lower_shape = layout_waveguide_from_points(TOP, workingLayer, bendPoints, cPinW, braggBendRadius)
debugRegionUpper.insert(upper_shape.to_itype(layout.dbu))
debugRegionLower.insert(lower_shape.to_itype(layout.dbu))

debugCPW = debugRegionUpper - debugRegionLower
TOP.shapes(outputDebugLayer).insert(debugCPW)
TOP.shapes(outputLayer).insert(upper_shape)
TOP.shapes(outputLayer).insert(lower_shape)
